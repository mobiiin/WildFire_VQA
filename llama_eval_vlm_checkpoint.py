...
def patch_mllama_processor_for_vllm_compat() -> bool:
    ...
    def _compat_get_num_multimodal_tokens(self, *args, **kwargs):
        # Conservative fallback budget for vLLM planner. The exact value is
        # architecture-dependent; using a safely high constant avoids
        # under-budgeting and prevents init-time crashes.
        # Return list form because some vLLM builds index [0].
...
class VLLMRunner:
    def __init__(
        ...
        llm_kwargs: Dict[str, Any] = {
            ...
            "limit_mm_per_prompt": {"image": max(1, int(limit_mm_images))},
...
def main():
    ...
    ap.add_argument("--vllm-limit-mm-images", type=int, default=4, help="Maximum number of image slots per vLLM prompt.")
    ap.add_argument("--vllm-model-impl", default="auto", choices=["auto", "vllm", "transformers"], help="vLLM model implementation mode. Use 'vllm' to forbid transformers fallback.")
    ap.add_argument("--disable-vllm-prefix-caching", action="store_true")
    ap.add_argument("--force-vllm-llama", action="store_true", help="Allow experimental Llama-3.2-Vision execution on vLLM instead of forcing Transformers fallback.")
    ...
    if args.backend == "vllm" and cfg.family == "llama" and not args.force_vllm_llama:
        print(
            "[WARN] vLLM backend is guarded for "
            f"{cfg.model_id}. Falling back to --backend transformers. "
            "Pass --force-vllm-llama to try the experimental vLLM path."
        )
    ...
    if args.backend == "vllm":
        ...
        if cfg.family == "llama":
            ...
            # Native Mllama support is not present in newer vLLM builds where
            # this architecture is routed to transformers backend.
            if args.vllm_model_impl == "vllm":
                try:
                    from vllm.model_executor.models import registry as vllm_registry

                    native_has_mllama = "MllamaForConditionalGeneration" in getattr(vllm_registry, "_VLLM_MODELS", {})
                except Exception:
                    native_has_mllama = False

                if not native_has_mllama:
                    raise RuntimeError(
                        "Strict --vllm-model-impl vllm requested, but installed vLLM has no native "
                        "MllamaForConditionalGeneration registry entry. This environment will always "
                        "fall back to transformers backend for Llama-3.2-Vision. "
                        "Use a vLLM version that still includes native Mllama support (commonly <= 0.10.1), "
                        "or change model."
                    )
        if cfg.family == "llama" and args.force_vllm_llama:
            print(
                "[WARN] Running experimental Llama-vLLM path. "
                "If generation fails, retry with --input-mode rgb_thermal (single concatenated image), "
                "--vllm-limit-mm-images 1, and/or smaller --vllm-max-model-len."
            )
        suppress_vllm_known_noisy_warnings()
        vllm_mm_images = args.vllm_limit_mm_images
        if cfg.family == "llama" and args.input_mode == "rgb_thermal":
            # RGB+thermal is concatenated into a single image for Llama below,
            # so one image slot is sufficient and generally more stable.
            vllm_mm_images = 1
        runner = VLLMRunner(
            cfg,
            ...
            limit_mm_images=vllm_mm_images,
