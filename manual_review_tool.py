...
class EvalLogger:
    ...
    def record_correct(self, qa: QAItem):
    ...
    def record_corrected(self, qa: QAItem, option_index: int):
        qa_key = make_qa_key(qa)
        now = time.strftime("%Y-%m-%d %H:%M:%S")
        meta = self._meta(qa)
...
class ReviewerUI:
    ...
    # -----------------------------
    # Label actions (question-centric auto-advance to NEXT IMAGE)
    # -----------------------------
    ...
    def choose_option(self, option_index: int):
        """
        NEW BEHAVIOR:
        - If human-chosen option == model answer (normalized), record as CORRECT (not corrected).
        - If different, record as CORRECTED.
        This lets evaluators always click the correct option without checking the model first.
        """
        self.clamp_indices()
        qa = self.current_list()[self.qa_i]
        if not qa.options:
        ...
        if norm_text(human_answer) == norm_text(qa.answer):
            # Treat as correct (agreement)
            self.logger.record_correct(qa)
        else:
            # True correction
    ...
    def _prev_question_internal(self) -> bool:
        ...
        if self.img_i > 0:
            self.img_i -= 1
            self.qa_i = len(self.current_list()) - 1
            return True
        return False
    ...
    def render(self):
        try:
            ...
            if done:
                ...
                if st.get("label") == LABEL_CORRECT:
                    status = "STATUS: ✅ human == model (correct)"
                elif st.get("label") == LABEL_CORRECTED:
                    status = f"STATUS: ❌ human != model (corrected to: {st.get('human_answer','')})"
                else:
            ...
            body = (
                ...
                "LABEL: c=Mark correct (auto-advances to NEXT IMAGE, same question)\n"
                "HUMAN ANSWER: press 1..6 or click an option (auto-advances to NEXT IMAGE, same question)\n"
                "NOTE: If the selected option matches the model answer, it is counted as CORRECT.\n"
                "SAVE: s=write report now | QUIT: q/Esc\n"
                f"(skip_done={self.skip_done}, auto_advance={self.auto_advance}) | Backend: {plt.get_backend()}\n"
            )
...
def main():
    ...
    src.add_argument("--json_dir", help="Directory containing multiple JSON files (loads all *.json)")

    ap.add_argument("--sample_pct", type=float, default=10.0, help="Percent of UNIQUE images to sample (0-100 or 0-1).")
    ...
    ap.add_argument("--out_dir", type=str, default="manual_eval_outputs")
    ap.add_argument("--run_name", type=str, default="manual_eval")
    ap.add_argument("--eval_checkpoint", type=str, default=None, help="Existing checkpoint.json to resume from.")
    ap.add_argument("--skip_done", action="store_true", help="Skip already-evaluated QA items (from checkpoint).")
    ap.add_argument("--no_auto_advance", action="store_true", help="If set, labeling won't auto-advance.")
    ap.add_argument("--max_option_buttons", type=int, default=6, help="Max option buttons shown (default 6).")
    args = ap.parse_args()
    ...
    order, groups = group_by_image(items)
    ...
    if n_pick == 0:
        raise SystemExit("sample_pct resulted in 0 images. Increase --sample_pct.")
    ...
    print(f"Loaded {total_rows} QA rows across {n_total_images} unique images.")
    print(f"Sampling {len(sampled_ordered)} images ({pct:.2f}%). Seed={args.seed}.")
    print(f"Backend: {plt.get_backend()} | max_dim={args.max_dim} | cache_size={args.cache_size}")
    print(f"Outputs -> out_dir: {args.out_dir} | run_name: {args.run_name}")
    if args.eval_checkpoint:
        print(f"Resume checkpoint: {args.eval_checkpoint}")
    if args.skip_done:
        print("skip_done: ON (will jump over already evaluated items)")
    ...
    ui._total_rows = total_rows
    ui._total_images = n_total_images

    plt.show()

    report_path = logger.write_report(total_rows=total_rows, total_unique_images=n_total_images)
    print(f"[FINAL] Report: {report_path}")
    print(f"[FINAL] Actions JSONL: {logger.actions_jsonl}")
    print(f"[FINAL] Checkpoint JSON: {logger.checkpoint_json}")
