...
# ===================================================
# Auto-detect available GPUs and limit to first GPU if not already set
if [ -z "$CUDA_VISIBLE_DEVICES" ]; then
    # Check how many GPUs are available
    GPU_COUNT=$(nvidia-smi --list-gpus 2>/dev/null | wc -l)
    if [ "$GPU_COUNT" -ge 1 ]; then
        export CUDA_VISIBLE_DEVICES="0"
        echo "Auto-detected $GPU_COUNT GPU(s). Setting CUDA_VISIBLE_DEVICES=0"
    else
    ...
    for MODE in "${INPUT_MODES[@]}"; do
        for REPEAT in "${REPEAT_PROMPTS[@]}"; do
            for TEMP in "${TEMP_SUMMARIES[@]}"; do
                ...
                CMD=("$PYTHON_BIN" "eval_vlm_checkpoint.py" "--input" "$INPUT_DIR" "--model" "$MODEL" "--input-mode" "$MODE" "--percent" "$PERCENT")
