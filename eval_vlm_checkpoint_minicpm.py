...
def configure_runtime_cache_dirs(outdir: str) -> Dict[str, str]:
    """
    Ensure vLLM/Torch/Triton use writable cache/temp directories.
    This avoids failures when inherited SLURM scratch paths are stale or inaccessible.
    """
    runtime_root = os.path.join(os.path.abspath(outdir), ".runtime_cache")
    tmp_dir = os.path.join(runtime_root, "tmp")
    inductor_dir = os.path.join(runtime_root, "torchinductor")
    triton_dir = os.path.join(runtime_root, "triton")
    vllm_cache_root = os.path.join(runtime_root, "vllm")

    for d in [runtime_root, tmp_dir, inductor_dir, triton_dir, vllm_cache_root]:
        os.makedirs(d, exist_ok=True)
    ...
    for key, fallback in selected.items():
        current = os.environ.get(key)
        if not _is_writable_dir(current):
            os.environ[key] = fallback

    return {
        "runtime_root": runtime_root,
        "tmp_dir": os.environ.get("TMPDIR", tmp_dir),
        "torchinductor_cache_dir": os.environ.get("TORCHINDUCTOR_CACHE_DIR", inductor_dir),
        "triton_cache_dir": os.environ.get("TRITON_CACHE_DIR", triton_dir),
        "vllm_cache_root": os.environ.get("VLLM_CACHE_ROOT", vllm_cache_root),
    }
...
class LlavaRunner:
    def __init__(self, model_id: str, device: str = "cuda", load_4bit: bool = False):
...
class QwenRunner:
    def __init__(self, model_id: str, use_flash_attn2: bool = False):
...
class GenericVLMRunner:
    ...
    def __init__(self, model_id: str, device: str = "cuda", load_4bit: bool = False, use_flash_attn2: bool = False):
...
class VLMRunner:
    def __init__(self, cfg: VLMConfig, device: str, load_4bit: bool, use_flash_attn2: bool):
        self.cfg = cfg
        if cfg.family == "llava":
            self.impl = LlavaRunner(cfg.model_id, device=device, load_4bit=load_4bit)
        elif cfg.family == "qwen":
            self.impl = QwenRunner(cfg.model_id, use_flash_attn2=use_flash_attn2)
        elif cfg.family in ["llama", "internvl", "minicpm", "pixtral"]:
            # Use generic runner for newer VLMs
            self.impl = GenericVLMRunner(cfg.model_id, device=device, load_4bit=load_4bit, use_flash_attn2=use_flash_attn2)
        else:
...
class VLLMRunner:
    def __init__(
        ...
        def _filter_supported_llm_kwargs(kwargs: Dict[str, Any]) -> Dict[str, Any]:
            try:
            ...
            if dropped:
                print(f"[WARN] Dropping unsupported vLLM args for this version: {dropped}")
            return filtered

        is_minicpm = (cfg.family == "minicpm")
        llm_kwargs: Dict[str, Any] = {
            ...
            "allowed_local_media_path": os.environ.get("TMPDIR", os.getcwd()),
        }
        if is_minicpm:
        ...
        self.cfg = cfg
        llm_kwargs = _filter_supported_llm_kwargs(llm_kwargs)
        try:
            self.llm = LLM(**llm_kwargs)
        except Exception as e:
            msg = str(e)
            pattern = r"Free memory on device .*\(([0-9.]+)/([0-9.]+) GiB\).*desired GPU memory utilization \(([0-9.]+),"
            m = re.search(pattern, msg)
            if m:
                free_gib = float(m.group(1))
                total_gib = float(m.group(2))
                requested_util = float(llm_kwargs.get("gpu_memory_utilization", 0.9))
                ratio = (free_gib / total_gib) if total_gib > 0 else 0.0
                retry_util = max(0.05, min(requested_util, ratio * 0.90))

                if retry_util < requested_util:
                    print(
                        "[WARN] vLLM startup memory check failed with "
                        f"gpu_memory_utilization={requested_util:.3f}. "
                        f"Retrying once with gpu_memory_utilization={retry_util:.3f}."
                    )
                    llm_kwargs["gpu_memory_utilization"] = retry_util
                    self.llm = LLM(**_filter_supported_llm_kwargs(llm_kwargs))
                else:
                    raise
            elif "Engine core initialization failed" in msg:
                retry_kwargs = dict(llm_kwargs)
                changed = False

                if not bool(retry_kwargs.get("enforce_eager", False)):
                    retry_kwargs["enforce_eager"] = True
                    changed = True

                cur_util = float(retry_kwargs.get("gpu_memory_utilization", 0.9))
                safe_util = min(cur_util, 0.75)
                if safe_util < cur_util:
                    retry_kwargs["gpu_memory_utilization"] = safe_util
                    changed = True

                if changed:
                    print(
                        "[WARN] vLLM engine initialization failed. "
                        "Retrying once with safer settings: "
                        f"enforce_eager={retry_kwargs.get('enforce_eager')} "
                        f"gpu_memory_utilization={retry_kwargs.get('gpu_memory_utilization')}."
                    )
                    self.llm = LLM(**_filter_supported_llm_kwargs(retry_kwargs))
                else:
                    raise
            elif "unexpected keyword argument" in msg or "got an unexpected keyword argument" in msg:
                fallback_kwargs = dict(llm_kwargs)
                for k in ["model_impl", "allowed_local_media_path", "enable_prefix_caching", "limit_mm_per_prompt"]:
                    fallback_kwargs.pop(k, None)
                print("[WARN] vLLM rejected one or more kwargs. Retrying with a minimal compatible arg set.")
                self.llm = LLM(**_filter_supported_llm_kwargs(fallback_kwargs))
            else:
                raise
        self.processor = AutoProcessor.from_pretrained(cfg.model_id, trust_remote_code=True)
        self.media_cache_dir = os.path.join(
            os.environ.get("TMPDIR", tempfile.gettempdir()),
            "eval_vlm_media_cache",
        )
        safe_mkdir(self.media_cache_dir)
        self._image_file_cache: "OrderedDict[int, str]" = OrderedDict()
        self._image_file_cache_limit = 4096

    def _build_minicpm_vllm_prompt(
        self,
        ...
        system_context = build_system_context(input_mode, temp_summary)
        task_before = build_task_instruction(
            question,
            options,
            input_mode,
            temp_summary,
            is_second_prompt=False,
        )

        parts: List[str] = []
        if system_context:
            parts.append(system_context)
        parts.append(task_before)
        parts.append("\n".join(["(<image>./</image>)" for _ in images]))

        if repeat_prompt:
            task_after = build_task_instruction(
                question,
                options,
                input_mode,
                temp_summary,
                is_second_prompt=True,
            )
            parts.append(task_after)
    ...
    def _build_prompt_and_mm(
        ...
        if self.cfg.family == "minicpm":
            prompt = self._build_minicpm_vllm_prompt(
                images=images,
                question=question,
                options=options,
        ...
        content_block = build_content_block(
            images,
            question,
            options,
            input_mode,
        ...
        try:
            ...
            prompt = build_fallback_text_prompt(
                question,
                options,
                input_mode,
                temp_summary,
    ...
    def _get_cached_image_url(self, image: Image.Image) -> str:
        key = id(image)
        cached_path = self._image_file_cache.get(key)
        if cached_path and os.path.exists(cached_path):
            self._image_file_cache.move_to_end(key)
            return f"file://{cached_path}"

        fd, path = tempfile.mkstemp(suffix=".png", dir=self.media_cache_dir)
        os.close(fd)
        image.save(path, format="PNG")
        self._image_file_cache[key] = path
        self._image_file_cache.move_to_end(key)

        while len(self._image_file_cache) > self._image_file_cache_limit:
            _, old_path = self._image_file_cache.popitem(last=False)
            try:
    ...
    def _build_chat_messages(
        ...
        for block_item in content_block:
            if block_item["type"] == "text":
                content.append({"type": "text", "text": block_item["text"]})
            elif block_item["type"] == "image":
                if image_idx >= len(images):
                    continue
                image_url = self._get_cached_image_url(images[image_idx])
                content.append(
                    {
                        "type": "image_url",
    ...
    def run_batch(self, batch_inputs: List[Dict[str, Any]], max_new_tokens: int, repeat_prompt: bool = False) -> List[str]:
        ...
        for bi in batch_inputs:
            msgs = self._build_chat_messages(
                images=bi["images"],
                question=bi["question"],
                options=bi["options"],
                input_mode=bi["input_mode"],
        ...
        sp = SamplingParams(temperature=0.0, top_p=1.0, max_tokens=max_new_tokens)
        outs = self.llm.chat(messages=messages_batch, sampling_params=sp, use_tqdm=False)
...
def main():
    ...
    runtime_cache_info = configure_runtime_cache_dirs(args.outdir)
    ...
    run_config = {
        ...
        "runtime_cache": runtime_cache_info,
        "run_name": run_name,
    ...
    if args.backend == "vllm":
        ...
        print(
            "[INFO] Runtime cache dirs: "
            f"TMPDIR={runtime_cache_info['tmp_dir']} | "
            f"TORCHINDUCTOR_CACHE_DIR={runtime_cache_info['torchinductor_cache_dir']} | "
            f"TRITON_CACHE_DIR={runtime_cache_info['triton_cache_dir']}"
        )
        runner = VLLMRunner(
            cfg,
        ...
        runner = VLMRunner(cfg, device=args.device, load_4bit=args.load_4bit, use_flash_attn2=args.use_flash_attn2)

    skip_missing_fields, skip_missing_files, exceptions, printed_ex = 0, 0, 0, 0
    missing_examples: List[Tuple[str, str]] = []
    image_cache: "OrderedDict[Tuple[str, str, str, int, bool], List[Image.Image]]" = OrderedDict()
    image_cache_limit = 512
        ...
        for jf, item in global_tasks:
            ...
            try:
                cache_key = (
                    ...
                    str(args.input_mode),
                    int(args.max_side),
                    bool(cfg.family == "llama" and args.input_mode == "rgb_thermal"),
                )
                images = image_cache.get(cache_key)
                if images is None:
                    images = load_input_images(rgb_path, thermal_path, mode=args.input_mode, max_side=args.max_side)
