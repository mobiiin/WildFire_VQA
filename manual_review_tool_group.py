...
def main():
    ...
    ap.add_argument("--group_map", type=str, help="JSON file from group_images.py to enable auto-propagation.")
    
    ap.add_argument("--sample_pct", type=float, default=10.0, help="Percent of UNIQUE images to sample (0-100).")
    ...
    print(f"Loaded {len(items)} QA rows across {n_total_images} unique images.")
    ...
    if group_map:
        print("Auto-propagation: ON (duplicate frames will be labeled automatically)")
    ...
    ui = ReviewerUI(
        ...
        group_map=group_map,
        all_items=items  # Pass ALL items so we can index them for propagation
    )
    ui._total_rows = len(items)
    ui._total_images = n_total_images

    plt.show()

    report_path = logger.write_report(total_rows=len(items), total_unique_images=n_total_images)
